...
export async function POST() {
  try {
    ...
    // Create Users
    const { data: owner1 } = await supabase.from('users').insert({ name: 'Rajesh Kumar', email: 'rajesh@stayease.in', phone: '+919876543210', role: 'OWNER', gender: 'MALE', is_verified: true, avatar: 'https://api.dicebear.com/9.x/avataaars/svg?seed=Rajesh' }).select().single();
    const { data: owner2 } = await supabase.from('users').insert({ name: 'Priya Sharma', email: 'priya@stayease.in', phone: '+919876543211', role: 'OWNER', gender: 'FEMALE', is_verified: true, avatar: 'https://api.dicebear.com/9.x/avataaars/svg?seed=Priya' }).select().single();
    const { data: owner3 } = await supabase.from('users').insert({ name: 'Amit Patel', email: 'amit@stayease.in', phone: '+919876543212', role: 'OWNER', gender: 'MALE', is_verified: true, avatar: 'https://api.dicebear.com/9.x/avataaars/svg?seed=Amit' }).select().single();
    ...
    await supabase.from('users').insert({ name: 'Admin User', email: 'admin@stayease.in', phone: '+919999999999', role: 'ADMIN', gender: 'MALE', is_verified: true, avatar: 'https://api.dicebear.com/9.x/avataaars/svg?seed=Admin' });
